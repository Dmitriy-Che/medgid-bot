...
# Обновляем ссылки на канал
CHANNEL_URL = "https://t.me/medgid_mo"
CHANNEL_USERNAME = "@medgid_mo"
...
def get_start_keyboard():
    ...
    builder.add(KeyboardButton(text="📢 Перейти в наш канал"))
    builder.add(KeyboardButton(text="📤 Поделиться ботом"))
    builder.adjust(1)
    return builder.as_markup(resize_keyboard=True)
...
# ------------------ ОБРАБОТЧИКИ КНОПОК КАНАЛА И ПОДЕЛИТЬСЯ ------------------
@dp.message(F.text == "📢 Перейти в наш канал")
async def handle_channel_link(message: types.Message):
    """Обработчик кнопки перехода в канал"""
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👉 Подписаться на канал", url=CHANNEL_URL)]
    ])
    await message.answer(
        "📢 <b>Подписывайтесь на наш канал МедГид МО!</b>\n\n"
        "Здесь вы найдете:\n"
        "✅ Новости медицины Подмосковья\n"
        "✅ Советы врачей и специалистов\n" 
        "✅ Акции и скидки клиник\n"
        "✅ Полезные медицинские статьи\n"
        "✅ Обзоры медицинских услуг в Домодедово",
        parse_mode="HTML",
        reply_markup=keyboard
    )

@dp.message(F.text == "📤 Поделиться ботом")
async def handle_share_bot(message: types.Message):
    """Обработчик кнопки поделиться ботом"""
    bot_username = (await bot.get_me()).username
    share_text = (
        "🤖 Нашел полезного бота для поиска врачей в Домодедово!\n\n"
        "🔹 Ищет лучших специалистов по рейтингу\n"
        "🔹 Анализирует симптомы и подсказывает к кому обратиться\n"
        "🔹 Показывает цены, контакты и отзывы\n\n"
        f"Попробуй: @{bot_username}"
    )
    
    # Создаем кнопку для легкого шаринга
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📤 Поделиться с друзьями", 
                             url=f"https://t.me/share/url?url=@{bot_username}&text={share_text}")]
    ])
    
    await message.answer(
        "📤 <b>Поделитесь ботом с друзьями!</b>\n\n"
        "Помогите им найти хороших врачей в Домодедово. "
        "Просто нажмите кнопку ниже для быстрой отправки в Telegram.",
        parse_mode="HTML",
        reply_markup=keyboard
    )
...
async def cmd_force_update(message: types.Message):
    """Принудительное обновление клавиатуры у всех пользователей"""
    if message.from_user.id != ADMIN_ID:
        return
    ...
    for user in users:
        try:
            ...
            await bot.send_message(
                ...
                parse_mode="HTML",
                reply_markup=get_start_keyboard()  # ← НОВАЯ КЛАВИАТУРА
            )
            updated += 1
            await asyncio.sleep(0.1)
        except Exception as e:
            logger.error(f"Ошибка обновления у {user['id']}: {e}")
            failed += 1
            continue
    
    await message.answer(
        f"✅ Принудительное обновление завершено!\n"
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    # Явно указываем reply_markup для принудительного обновления
    try:
        with open("start.jpg", "rb") as img:
            await bot.send_photo(
                ...
                caption=caption, 
                parse_mode="HTML", 
                reply_markup=get_start_keyboard()  # ← ЯВНО УКАЗЫВАЕМ
            )
    except Exception:
        await message.answer(
            caption, 
